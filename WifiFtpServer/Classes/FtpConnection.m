...
// ----------------------------------------------------------------------------------------------------------
{
    ...
    if(server.notificationObject && [server.notificationObject respondsToSelector:@selector(getIsAnonymous)])
    {
        BOOL anonymous = [server.notificationObject getIsAnonymous];
        if (!anonymous)
        {
            NSString* userName = [server.notificationObject getFtpUserName];
            if (![currentUser isEqualToString:userName])
            {
                NSString *outputString = [ NSString stringWithFormat:@"332 User name required %@", currentUser ];
...
// ----------------------------------------------------------------------------------------------------------
{
    ...
    if(server.notificationObject && [server.notificationObject respondsToSelector:@selector(getIsAnonymous)])
    {
        ...
        if (!anonymous)
        {
            NSString* password = [server.notificationObject getFtpPWD];
            if (![pass isEqualToString:password])
            {
                NSString *outputString = [ NSString stringWithFormat:@"530 User %@ not logged in.", currentUser ];
                [ sender sendMessage:outputString];
                return;
            }
        }
...
// ----------------------------------------------------------------------------------------------------------
{
    ...
    if ([cmd isEqualToString:@"utf8"])
    {
        //opts utf8 on
        //不返回200的话，中文会乱码
        cmdstr = [ NSString stringWithFormat:@"200 okay '%@'",cmd ];
    }
